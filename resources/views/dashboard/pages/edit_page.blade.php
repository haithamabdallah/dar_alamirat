                        ...
                        @foreach (Config('language') as $key => $lang)
                            <div class="col-6 my-4">
                                ...
                                <input class="form-control" type="text" name="name[{{ $key }}]"
                                    value="{{ $page->getTranslation('name', $key) ?? old('name.' . $key) }}" />
                                @error('name.' . $key)
                        ...
                        <div class="col-12 my-4">
                            <div>
                                ...
                                <div class="form-control p-0 overflow-hidden">
                                    <textarea class="textarea form-control wysihtml5" name="content[{{ $key }}]" placeholder="Enter text ..."
                                        rows="12">{{ $page->getTranslation('content', $key) ?? old('content.' . $key) }}</textarea>
                                </div>
                                @error('content.' . $key)
                                    <span class="text-danger" role="alert">
                                        <strong>{{ $message }}</strong>
                    ...
                    <div class="col-12 ">
                        <div class="m-3">
                            ...
                            <select class="form-control" name="priority" required>
                                ...
                                @for ($i = 0; $i <= 10; $i++)
                                    <option value="{{ $i }}"
                                        @if ($page->priority == $i) selected @endif>{{ $i }}</option>
                                @endfor
                            </select>
                            @error('priority')
                                <span class="text-danger" role="alert">
